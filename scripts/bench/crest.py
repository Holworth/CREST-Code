...
class CrestMemoryNode(Node): 
  ...
  def run(self, workload:str, thread_num:int, coro_num:int) -> bool:
    ...
    cmd = "source ~/.zshrc && cd {} && ./bench_runner --type=mn --id={} ".format(binpath, self.id) \
        + "--config=../../config/{}_config.json --workload={} ".format(workload, workload) \
        + "--threads={} --coro={} --replay=true > output 2>&1 &".format(thread_num, coro_num)
...
class CrestComputeNode(Node): 
  ...
  def run(self, workload: str, thread_num: int, coro_num: int) -> bool: 
    ...
    cmd = "source ~/.zshrc && cd {} && ".format(self.bin_path) \
          + "numactl --cpunodebind={} --membind={} ".format(self.numa_node, self.numa_node) \
          ...
          + "> output 2&>1".format(thread_num, coro_num) 
    t = self.exec_cmd(cmd)
    if t is False:
      self.shutdown()
    return t
...
class CrestBenchmark:
  def __init__(self, workload: str, cns: List[Tuple[str, int]], mns: List[Tuple[str, int]]):
  ...
  def clean_bench_results(self):
    for n in self.cns:
      n.exec_cmd("rm -rf {}/bench_results/{}".format(n.project_path, self.workload))
  ...
  def setup_ycsb_configuration(self, key_num:int, theta:float, op_num:int, write_ratio:int = 100):
  ...
  def print_run_header(self, thread_num:int, coro_num:int): 
      if self.workload == "tpcc":
          print("[=] Run CREST TPCC Benchmark: {} Threads, {} Coros".format(thread_num, coro_num))
      if self.workload == "smallbank":
          print("[=] Run CREST SmallBank Benchmark: {} Threads, {} Coros".format(thread_num, coro_num))
      if self.workload == "ycsb":
          print("[=] Run CREST YCSB Benchmark: {} Threads, {} Coros".format(thread_num, coro_num))
  ...
  def launch_compute_pool(self, thread_num:int, coro_num:int):
    ...
    res = ""
    if self.workload == "tpcc":
      res = "CREST {} Benchmark: Warehouse[{}], Threads[{}], Coroutines[{}]".format(self.workload, self.num_warehouse, thread_num, coro_num)
    if self.workload == "smallbank":
      res = "CREST {} Benchmark: Accounts[{}], Zipfian[{}], Threads[{}], Coroutines[{}]".format(self.workload, self.num_accounts, self.theta, thread_num, coro_num)
    if self.workload == "ycsb":
      res = "CREST {} Benchmark: NumKeys[{}], Zipfian[{}], Threads[{}], Coroutines[{}]".format(self.workload, self.num_key, self.theta, thread_num, coro_num)
  ...
  def create_detail_dir(self, thread_num: int, coro_num: int): 
    if self.workload == "tpcc":
      self.detail_dir = "crest_tpcc_detail_warehouse_{}_threads_{}_coro_{}".format(self.num_warehouse, thread_num, coro_num)
    if self.workload == "smallbank":
      self.detail_dir = "crest_smallbank_detail_accounts_{}_zipfian_{}_threads_{}_coro_{}".format(self.num_accounts, self.theta, thread_num, coro_num)
    if self.workload == "ycsb":
      self.detail_dir = "crest_ycsb_detail_keys_{}_zipfian_{}_ops_{}_threads_{}_coro_{}".format(self.num_key, self.theta, self.num_op, thread_num, coro_num)
